...
def get_vacancies(language,token):
    ...
    params={'town':4,'catalogues':48,'keyword':'{}'.format(language),'count':100}
...
def get_avarage_salary(vacancies):
    ...
    nonempty_salary = [salary for salary in avarage_salary if salary != None]
...
def get_statistic_salary():
    ...
    for language in languages:
        ...
        salarys = get_avarage_salary(vacancies)
        avarage_salary = int(sum(salarys)/len(salarys)) if sum(salarys) > 0 else 0
        count = len(salarys)
        amount = all_vacancies[language]['total']
        print(language,salarys,amount)
        avarage_stattistic[language] = {"vacancies_found":amount,"vacancies_processed":count,"average_salary":avarage_salary}
